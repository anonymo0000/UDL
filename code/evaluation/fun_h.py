...
def merge_s(sentence_list):
    with open('mergeprompt.txt','r',encoding='utf8') as f:
        prompt = f.read()
        
    prompt = prompt + "Task:\ninput:\nsentence_list:" + str(sentence_list) 
    prompt = prompt + '\ninformation entropy: ' + str(round(information_entropy(sentence_list),3)) +'\noutput:\n'
    
    res = ask_LLM(prompt)  
    return res
    
        
def information_entropy(sentence_list):        
    words = []
...
def extract_special_voc(sentence):
...
def extract_special_voc1(sentence):

    return sentence


def similay_BERT(elements):    
